...
def layout_extraction(opencv_image):
    ...
    model = lp.PaddleDetectionLayoutModel(config_2)
    layout = model.detect(image)
    return layout

def table_generation(image_layout,opencv_image):
    ...
    tables = lp.Layout([b for b in image_layout if b.type=='Table'])
    for i, table in enumerate(tables):
        cropped_image = opencv_image[int(table.block.y_1):int(table.block.y_2), int(table.block.x_1):int(table.block.x_2)]
        gray = cv2.cvtColor(cropped_image, cv2.COLOR_BGR2GRAY)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(16, 16))
        enhanced = clahe.apply(gray)
        location = os.path.join(output_folder,f'table_{i+1}.png')
...
def new_image_processing(x):
    ...
    print(x.image.path)
    image_layout = layout_extraction(img)
...
def page_to_document(file:FilePage):
    ...
    for item in all_cols:
        ...
        if item[1] == 'Figure':
            img = Image.fromarray(roi).convert('BGR')
            img_io = BytesIO()
